    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md animate-fade-in">
      <div className="bg-slate-900 border border-slate-700/80 rounded-3xl p-8 w-full max-w-md flex flex-col gap-6 shadow-2xl shadow-black/60 animate-scale-up">
        ...
        <div className="flex gap-4 justify-center">
          <AccuracyBadge
            value={event.white_accuracy}
            label="Akurasi Putih"
          />
          <AccuracyBadge
            value={event.black_accuracy}
            label="Akurasi Hitam"
          />
        </div>
...
function ChessAnalyzerApp() {
  ...
  // Manage loading state and highlights based on whose turn it is
  useEffect(() => {
  ...
  const handleMoveResult = (result: {
    ...
    // Game-over received from WebSocket via Board
    if (result.game_over) {
      setIsGameEnded(true);
  ...
  const getGameStatusLabel = () => {
    if (isGameEnded) {
      ...
      if (r === '1-0') return 'Putih Menang ♔';
      if (r === '0-1') return 'Hitam Menang ♚';
      if (r === '1/2-1/2') return 'Remis 🤝';
      return 'Game Selesai';
    }
