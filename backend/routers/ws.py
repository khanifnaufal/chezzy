...
async def websocket_game(websocket: WebSocket, session_id: str):
    ...
    try:
        try:
            engine = get_engine()
            logger.info(f"Started persistent Stockfish engine for WS session: {session_id}")
        except Exception as engine_err:
            logger.error(f"Failed to start persistent Stockfish engine for session {session_id}, falling back: {engine_err}")
        ...
        if is_player_turn:
            try:
                recs = recommend_moves(board.fen(), is_white, engine=engine)
        ...
        try:
            while True:
                ...
                # ----------------------------------------------------------------
                if msg.type == "resign":
                    logger.info(f"Resign request received: session_id={session_id}, color={msg.color}")
                    resign_color = msg.color or player_color
                ...
                # ----------------------------------------------------------------
                if msg.type == "undo":
                    count = msg.count or 1
                    logger.info(f"Undo request received: session_id={session_id}, count={count}")
                    ...
                    if undone_count > 0:
                        db = _get_db()
                        try:
                            # Find the last undone_count moves
                            last_moves = db.query(MoveRecord).filter(
                                MoveRecord.game_id == session_id
                            ).order_by(MoveRecord.id.desc()).limit(undone_count).all()
                            
                            for lm in last_moves:
                                db.delete(lm)
                            
                            # Reactivate session and clear game result
                            session_row = db.query(SessionModel).filter(SessionModel.id == session_id).first()
                            if session_row:
                                session_row.current_fen = board.fen()
                            ...
                            if game_row:
                                ...
                                game_row.pgn_raw = None
                                game_row.white_accuracy = None
                                game_row.black_accuracy = None
                            
                            # Update active_move_counts
                            ply = active_move_counts.get(session_id, 0)
                            active_move_counts[session_id] = max(0, ply - undone_count)
                            
                            db.commit()
                        except Exception as e:
                            db.rollback()
                            logger.error(f"Failed to delete undone moves from DB: {e}", exc_info=True)
                        finally:
                            db.close()
                    
                    # Recalculate recommendations for new position
                    recs = []
                    ...
                    if is_next_player_turn:
                        try:
                            recs = recommend_moves(board.fen(), next_is_white, engine=engine)
                        except Exception as e:
                            logger.error(f"Error calculating recommendations after undo: {e}")
                    
                    score_after = 0
                    try:
                        score_after = get_evaluation(board.fen(), engine=engine)
                    except Exception as e:
                        logger.error(f"Error evaluating position after undo: {e}")
                    
                    active_threat = None
                    if len(board.move_stack) > 0:
                        last_move = board.move_stack[-1]
                        last_move_is_white = (board.turn != chess.WHITE)
                        last_move_is_opponent = (
                            (player_color == "white" and not last_move_is_white)
                            or (player_color == "black" and last_move_is_white)
                        )
                        if last_move_is_opponent:
                            threat_str = detect_opponent_threats(board, last_move, player_color)
                            best_response_str = ""
                            if recs:
                                best_response_str = recs[0]["move_san"]
                            else:
                                try:
                                    top_moves = get_top_moves(board.fen(), n=1, engine=engine)
                                    if top_moves:
                                        best_response_str = top_moves[0]["move_san"]
                                except Exception as e:
                                    logger.error(f"Error calculating best response: {e}")
                            if threat_str:
                                active_threat = {
                                    "threat": threat_str,
                    ...
                    if len(board.move_stack) > 0:
                        db = _get_db()
                        try:
                            last_record = db.query(MoveRecord).filter(
                                MoveRecord.game_id == session_id
                            ).order_by(MoveRecord.id.desc()).first()
                            if last_record:
                                last_move_eval = {
                                    "san": last_record.san,
                                    "label": last_record.label or "",
                                    "explanation": last_record.explanation or "",
                                }
                        except Exception as e:
                            logger.error(f"Failed to fetch last move record after undo: {e}")
                        finally:
                            db.close()
                    ...
                    await websocket.send_json({
                        ...
                        "last_move_evaluation": last_move_eval,
                    })
                    continue
                ...
                score_before = None
                try:
                    score_before = get_evaluation(board.fen(), engine=engine)
                except Exception as e:
        ...
        if engine is not None:
            try:
                engine.quit()
                logger.info(f"Closed persistent Stockfish engine for WS session: {session_id}")
            except Exception as e:
                logger.error(f"Error quitting Stockfish engine for session {session_id}: {e}")
